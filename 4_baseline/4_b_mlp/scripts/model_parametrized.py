...
class MLP(pl.LightningModule):
    def __init__(self, input_dim, hidden_dims, activation, dpout, loss_f):
        ...
        for h in hidden_dims: 
            ...
            layers.append(nn.ReLU() if activation == "relu" else nn.Sigmoid()) # Sigmoid PPC = 0.2810 loss = 1.2403, con ReLu è avg.PCC = 0.3470, invece avg loss = 1.1788
            layers.append(nn.Dropout(dpout))  # Ridurre overfitting
            prev_dim = h

        layers.append(nn.Linear(prev_dim, 1))

        self.model = nn.Sequential(*layers) 
        self.loss_fn = nn.L1Loss() if loss_f == 'L1' else nn.MSELoss() # invece di MSE che penalizza errori grandi di più, mettiamo questo
